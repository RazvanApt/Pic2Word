...
class CSS(Dataset):
    ...
    def return_all(self, idx):
        ...
        logging.info(f"CSS Class token_texts: shape {token_texts.shape}; type {type(token_texts)}")
...
def get_imgnet_r(args, preprocess_fn, is_train, input_filename=None):
    if input_filename is None:
        input_filename = args.train_data if is_train else args.val_data
    assert input_filename
    # logging.info(f"Project path: {get_project_root()}")
    args.root_data = get_project_root()
    path_data = os.path.join(args.root_data, 'data/imgnet/imagenet-r')
